...
<TypeTable
  type={{
    ...
    SidebarCollapseThreshold: {
      description: 'Fraction (0-1) of the sidebar width that triggers compact mode when dragging',
      type: 'number',
      default: '0.5',
      required: false
    },
    SidebarHighlightCallback: {
      description: 'Callback invoked with `(dividerFrame, isActive)` whenever the resize handle should highlight',
      type: '(Frame, boolean) -> ()',
      default: 'nil',
      required: false
    }
...
If you’d prefer to style the divider highlight yourself, supply a callback. The divider `Frame` and the current state are passed in so you can animate, tween or recolor however you’d like:
...
| --- | --- | --- | --- |
| 1 | Desired sidebar width in pixels | number | `SidebarMinWidth` |
